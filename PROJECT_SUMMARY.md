...
这是一个面向中文与 English 用户的本地优先自我反思平台。它不要求用户先定义自己，而是从一个当下的问题或方向开始，完成一项测试，再把结果带回真实生活。游客无需账号；注册或登录后，本机与云端数据会自动合并同步。
...
账号由自托管 Next.js 服务和 SQLite 数据库支持。游客数据在当前设备本地保存；注册或登录后，本机数据会自动与云端合并同步。登录后可验证当前密码并设置新密码，改密会撤销其他设备会话；当前不
提供忘记密码邮件找回流程。
