    ...
    <div className="min-h-screen bg-[#FAFAF8] dark:bg-[#0a0a0a]">
      ...
      <section className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
        <div className="grid grid-cols-3 gap-4">
          <AnimatedNumber target={testCount} label={lang === "zh" ? "心理测试" : "Psych Tests"} icon="🧪" delay={0} />
          <AnimatedNumber target={9} label={lang === "zh" ? "测试维度" : "Dimensions"} icon="🧭" delay={200} />
          <AnimatedNumber target={4} label={lang === "zh" ? "内在世界" : "Inner Worlds"} icon="🌙" delay={400} />
        </div>
      </section>
      ...
      <section className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
        ...
        >
          {lang === "zh" ? "三步探索自己" : "Three Steps to Self-Discovery"}
        </motion.h2>
        <div className="grid grid-cols-3 gap-6">
          <HowItWorksStep
            icon="🎯"
            title={lang === "zh" ? "选择测试" : "Choose a Test"}
            desc={lang === "zh" ? "从 9 大维度 118 个测试中，选一个与你此刻相关的。" : "Pick from 118 tests across 9 dimensions — whatever speaks to you right now."}
            index={0}
          />
          <HowItWorksStep
            icon="✍️"
            title={lang === "zh" ? "回答问题" : "Answer Questions"}
            desc={lang === "zh" ? "诚实作答，没有对错之分。每道题都是一次自我对话。" : "Answer honestly — no right or wrong. Each question is a conversation with yourself."}
            index={1}
          />
          <HowItWorksStep
            icon="🪞"
            title={lang === "zh" ? "探索结果" : "Discover Results"}
            desc={lang === "zh" ? "看到你的内在画像，理解自己的模式与力量。" : "See your inner portrait. Understand your patterns and strengths."}
            index={2}
          />
        </div>
      </section>
      ...
      <section id="worlds" className="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-24">
        ...
        >
          <h2 className="text-2xl font-bold text-[#2C2C2C] dark:text-white sm:text-3xl">
            {lang === "zh" ? "四个内在世界" : "Four Inner Worlds"}
          </h2>
          <p className="mt-2 text-sm text-[#2C2C2C]/60 dark:text-white/60">
            {lang === "zh" ? "选择一个与你共振的世界，开始探索。" : "Choose a world that resonates with you and begin exploring."}
          </p>
        </motion.div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        ...
        >
          <h2 className="text-2xl font-bold text-[#2C2C2C] dark:text-white sm:text-3xl">{lang === "zh" ? "从这里开始" : "Start Here"}</h2>
          <p className="mt-2 text-sm text-[#2C2C2C]/60 dark:text-white/60">{lang === "zh" ? "不知道选什么？这几个测试最受欢迎。" : "Not sure where to begin? These are our most popular tests."}</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="mt-8 text-center">
          <a href="#explore" className="inline-flex h-10 items-center justify-center rounded-full border border-[#2C2C2C]/20 dark:border-white/20 px-6 text-sm font-medium text-[#2C2C2C] dark:text-white transition-colors hover:bg-[#2C2C2C]/5 dark:hover:bg-white/5">{lang === "zh" ? `查看全部 ${testCount} 个测试 →` : `Explore all ${testCount} tests →`}</a>
        </motion.div>
      </section>
      ...
      <footer className="border-t border-[#2C2C2C]/8 dark:border-white/8 px-4 py-16 text-center">
        <div className="mx-auto max-w-2xl">
          ...
          <p className="text-sm text-[#2C2C2C]/50 dark:text-white/50 mb-4">{lang === "zh" ? "这里不是诊断，而是一面帮助你靠近自己的镜子。" : "This isn't a diagnosis — it's a mirror to help you get closer to yourself."}</p>
          <div className="flex items-center justify-center gap-6 text-xs text-[#2C2C2C]/30 dark:text-white/30 flex-wrap">
            <Link href="/compat/" className="hover:text-[#2C2C2C]/60 dark:hover:text-white/60 transition-colors">{lang === "zh" ? "关系配对" : "Compatibility"}</Link>
            <span>·</span>
            <Link href="/history/" className="hover:text-[#2C2C2C]/60 dark:hover:text-white/60 transition-colors">{lang === "zh" ? "测试历史" : "History"}</Link>
            <span>·</span>
            <Link href="/bookmarks/" className="hover:text-[#2C2C2C]/60 dark:hover:text-white/60 transition-colors">{lang === "zh" ? "收藏" : "Bookmarks"}</Link>
            <span>·</span>
            <Link href="/compare/" className="hover:text-[#2C2C2C]/60 dark:hover:text-white/60 transition-colors">{lang === "zh" ? "对比" : "Compare"}</Link>
            <span>·</span>
            <Link href="/stats/" className="hover:text-[#2C2C2C]/60 dark:hover:text-white/60 transition-colors">{lang === "zh" ? "统计" : "Stats"}</Link>
            <span>·</span>
            <Link href="/trends/" className="hover:text-[#2C2C2C]/60 dark:hover:text-white/60 transition-colors">{lang === "zh" ? "趋势" : "Trends"}</Link>
            <span>·</span>
            <Link href="/changelog/" className="hover:text-[#2C2C2C]/60 dark:hover:text-white/60 transition-colors">{lang === "zh" ? "更新日志" : "Changelog"}</Link>
            <span>·</span>
