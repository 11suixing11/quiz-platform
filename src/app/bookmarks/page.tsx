  ...
  return (
    <div className="atlas-page min-h-screen">
      ...
      <PageContainer>
        ...
        {tests.length === 0 ? <div className="atlas-empty-state mt-12"><BookmarkX className="mx-auto size-8 text-accent" /><h2 className="mt-5 text-xl font-semibold">{language === "zh" ? "还没有收藏" : "Nothing saved yet"}</h2><p className="mt-3 max-w-sm text-sm leading-6 text-ink/50 dark:text-white/50">{language === "zh" ? "看到想以后再做的测评时，点一下收藏即可。" : "Save an assessment whenever you want to return later."}</p><Link href="/#library" className="atlas-primary-action mx-auto mt-7"><ListFilter className="size-4" />{language === "zh" ? "浏览精选测评" : "Browse assessments"}</Link></div> : <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{tests.map((test, index) => <TestCard key={test.id} test={test} index={index} lang={language} />)}</div>}
      </PageContainer>
    </div>
  );
}
