...
export default function ResultPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      // 1. Read stored result
      let stored: { result?: QuizResult; answers?: number[] } | null = null;
      try {
        const raw = localStorage.getItem(`quiz-result-${type}`);
        if (raw) stored = JSON.parse(raw);
      } catch { /* corrupt storage */ }

      // 2. Look up test entry from registry
      const entry = getTestById(type) ?? null;

      // 3. Load actual test data module
      const data = await loadTestData(type);

      // 4. Get result — prefer stored result, fall back to recalculation
  ...
  const handleRetake = useCallback(() => {
    try { localStorage.removeItem(`quiz-result-${type}`); } catch {}
    router.push(`/quiz/${type}`);
  }, [router, type]);

  const handleCopy = useCallback(async () => {
    const text = lang === "zh"
      ? `我在「认识你自己」完成了${registryEntry?.zh.name ?? type}测试，结果是：${heroTitle}。来试试吧！`
      : `I completed the ${registryEntry?.en.name ?? type} test on "Know Yourself" and got: ${heroTitle}. Try it!`;
    try {
      await navigator.clipboard.writeText(text);
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    } catch { /* clipboard blocked */ }
  }, [type, lang, registryEntry, heroTitle]);
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-4 min-h-screen px-6 text-center bg-[#FAFAF8]">
        ...
        <div className="flex gap-3">
          <Link href={`/quiz/${type}`} className={cn(buttonVariants({ variant: "default" }), "rounded-xl")}>
