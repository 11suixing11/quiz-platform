...
const copy = {
  zh: { summary: "当前摘要", records: "结果记录", bookmarks: "收藏", storage: "占用空间", export: "导出备份", exportDesc: "下载包含结果、收藏、偏好的 JSON 文件。", import: "导入备份", importDesc: "只接受本应用生成的 v3 JSON 备份。", merge: "合并", mergeDesc: "保留当前记录并添加新记录", replace: "替换", replaceDesc: "用备份完全替换当前数据", drop: "拖入 JSON 文件，或点击选择", choose: "选择文件", clear: "清空本地数据", clearDesc: "删除当前浏览器中的记录、收藏、会话和偏好。", confirm: "再次确认清空", cancel: "取消", success: "操作完成", noData: "当前没有可导出的数据", imported: "条记录已导入", invalid: "无法读取这个备份" },
  en: { summary: "Current snapshot", records: "Results", bookmarks: "Bookmarks", storage: "Storage", export: "Export backup", exportDesc: "Download a JSON file containing results, bookmarks, and preferences.", import: "Import backup", importDesc: "Only v3 backups created by this app are accepted.", merge: "Merge", mergeDesc: "Keep current records and add new ones", replace: "Replace", replaceDesc: "Fully replace current data with the backup", drop: "Drop a JSON file here, or click to choose", choose: "Choose file", clear: "Clear local data", clearDesc: "Delete records, bookmarks, sessions, and preferences from this browser.", confirm: "Confirm clear", cancel: "Cancel", success: "Done", noData: "There is no data to export yet", imported: "records imported", invalid: "This backup could not be read" },
} as const;
...
export function DataManager({ lang = "zh" }: { lang?: Lang }) {
  ...
  const refresh = useCallback(() => setSummary(getDataSummary()), []);
  useEffect(() => {
  ...
  const handleExport = () => {
    if (!summary.quizEntries && !summary.bookmarks) { setFeedback({ kind: "error", text: t.noData }); return; }
    exportAllData();
