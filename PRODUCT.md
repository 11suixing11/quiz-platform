...
- Assessment badges are derived on the server from a signed-in account's completed public assessments, so nothing is reported by the client and nothing can be forged. Each earned result variant joins the account's collection; the owner may wear at most three and must separately opt in before worn badges appear next to their name on community posts, comments, and image posts. Badges show result labels only — never scores or dimension values — and remain a chosen observation rather than an identity claim.
